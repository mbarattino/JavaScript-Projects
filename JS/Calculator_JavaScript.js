...
const Calculator = {
    ...
    Display_Value: '0',
    //this will hold the first opearnd for any expression we sit to null for now
    First_Operand: null,
    //this check whether or not the second operand has been input
    Wait_Second_Operand: false,
    //this will hold the oeprator we sit it to null for now
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    //we are checkign to see if Wait_second_operand is true and set
    //Display_Value to the key thta was clicked
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that is th edispay value does not contain a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //Calculator.First_Operand if it doesn't aleady exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second_Operand is true,
    //then updares the operator and exits from the function
